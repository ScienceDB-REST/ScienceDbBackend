...
if (config.use_env_variable) {
  var sequelize = si.initSequelize(process.env[config.use_env_variable],
    multDbConfig);
} else {
...
fs.readdirSync(__dirname)
  .filter(function(file) {
    return (file.indexOf('.') !== 0) && (file !== basename) && (file !==
      'multipleDatabaseSupport.js');
  })
  .forEach(function(file) {
    if (file.slice(-3) !== '.js') return;
    console.log('DEBUG: Model-File "' + file + '" uses Sequelize-Instance ' +
      (sequelize.mappings[file] || 'default'));
