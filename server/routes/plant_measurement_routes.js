...
router.get('/plant_measurements', acl.middleware(1),
    function(req, res) {
        models.
        plant_measurement.findAll(helper.searchPaginate(req,
...
// get CSV of all plant_measurements
router.get('/plant_measurements/csv_export', acl.middleware(1),
    function(req, res) {
        helper.csvExport(models.plant_measurement).then(function(csvStr) {
            res.set({
                'Content-Disposition': 'attachment; filename=plant_measurements.csv',
...
router.get('/plant_measurements/vue_table', acl.middleware(1),
    function(req, res) {
        helper.vueTable(req, models.plant_measurement,
...
router.post('/plant_measurements/upload_csv', acl.middleware(1),
    function(req, res) {
        ...
        req.files.csv_file.mv(tmpFile).then(() => {
            return helper.parseCsvStream(tmpFile, models.plant_measurement, delim, cols)
        }).then(() => {
